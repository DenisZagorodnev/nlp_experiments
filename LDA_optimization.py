...
extra_stopwords = txtprpc.get_freq_n_words(data['Текст'], n = 100)
...
#предобработали корпус всеми предложенными стратегиями (последоватльеностями функций)
...
for funcs_seq in funcs_seqs:
    ...
    X_trains.append(preproc_data(data, stopwords, extra_stopwords, 'Текст', funcs_seq))
...
data['predicted_clusters'] = [row[0] for row in np.asarray(prediction)]
